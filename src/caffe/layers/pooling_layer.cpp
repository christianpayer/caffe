  ...
  else
  {
    if (pool_param.has_kernel_h() || pool_param.has_kernel_w()) {
      ...
      CHECK(num_kernel_dims == 1 || num_kernel_dims == num_spatial_axes_)
        ...
        for (int i = 0; i < num_spatial_axes_; ++i) {
          kernel_shape_[i] = pool_param.kernel_size((num_kernel_dims == 1) ? 0 : i);
        }
    }
    for (int i = 0; i < num_spatial_axes_; ++i) {
...
void PoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  for (unsigned int i = 2; i < input_shape_.size(); ++i) {
    pooled_shape_[i] = static_cast<int>(std::ceil(static_cast<float>(
        input_shape_[i] + 2 * pad_[i - 2] - kernel_shape_[i - 2]) / stride_[i - 2])) + 1;
  }
  for (unsigned int i = 0; i < num_spatial_axes_; ++i) {
    if (pad_[i]) {
      // If we have padding, ensure that the last pooling starts strictly
      // inside the image (instead of at the padding); otherwise clip the last.
      if ((pooled_shape_[i] - 1) * stride_[i] >= input_shape_[i] + pad_[i]) {
        --pooled_shape_[i];
      }
      CHECK_LT((pooled_shape_[i] - 1) * stride_[i], input_shape_[i] + pad_[i]);
    }
  }
  // reshape outputs
